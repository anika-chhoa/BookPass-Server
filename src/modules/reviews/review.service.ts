...
export async function createReview(userId: string, bookId: string, rating: number, comment: string) {
  ...
  const doc: ReviewDoc = { bookId: new ObjectId(bookId), userId: new ObjectId(userId), userName: user.name, rating, comment, createdAt: new Date() };
  const { insertedId } = await reviewsCollection().insertOne(doc);

    const currentRating = Number.isFinite(book.rating) ? book.rating : 0;
  const currentReviewCount = Number.isFinite(book.reviewCount) ? book.reviewCount : 0;
